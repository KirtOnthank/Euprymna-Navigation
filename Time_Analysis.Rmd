...
```{r table0-linear-model-learning-curves, message=FALSE, warning=FALSE}
format_p <- function(p) {
...
lm_time_models <- lapply(paste0("P", 1:5), function(tr) {
  lm(Time.Observed..sec. ~ Run, data = squidtime[squidtime$Trial == tr, ])
})
lm_dist_models <- lapply(paste0("P", 1:5), function(tr) {
  lm(path_length_cm ~ Run, data = squiddist[squiddist$Trial == tr, ])
})

lm_pvals <- data.frame(
  Procedure = paste0("P", 1:5),
  Time_Slope = sapply(lm_time_models, function(m) round(coefficients(m)["Run"], 3)),
  Time_P = sapply(lm_time_models, function(m) summary(aov(m))[[1]]["Run", "Pr(>F)"]),
  Dist_Slope = sapply(lm_dist_models, function(m) round(coefficients(m)["Run"], 3)),
  Dist_P = sapply(lm_dist_models, function(m) summary(aov(m))[[1]]["Run", "Pr(>F)"])
)

kbl(
  data.frame(
    Procedure = lm_pvals$Procedure,
    Time_Slope = lm_pvals$Time_Slope,
    Time_P = format_p(lm_pvals$Time_P),
    Dist_Slope = lm_pvals$Dist_Slope,
...
![Within-trial Cox: log-relative hazard by run — time (5 panels)](LogRelHaz_byRun_5panels.png)
...
![Within-trial Cox: log-relative hazard by run — distance (5 panels)](LogRelHaz_Dist_byRun_5panels.png)
...
cox_within <- data.frame(
  Procedure = paste0("P", 1:5),
  Time_P = sapply(list(mP1, mP2, mP3, mP4, mP5), function(m) {
...
![P1 vs. P2: Cumulative proportion escaped with 95% CI](P1P2_Survival_CI.png)
...
![P1 vs. P2: Distance survival with 95% CI](P1P2dist_Survival_CI.png)
...
![P2 vs. P3/P4/P5: Time survival curves (3 panels)](P2_vs_P3P4P5_3panel.png)
...
![P2 vs. P3/P4/P5: Distance survival curves (3 panels)](P2_vs_P3P4P5_Dist_3panel.png)
